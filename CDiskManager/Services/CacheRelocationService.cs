...
public sealed class CacheRelocationService
{
    ...
    private static List<CacheDefinition> GetCacheDefinitions()
    {
        ...
        [
            ...
            new("QQ Cache_Data", Path.Combine(roaming, @"QQ\Cache\Cache_Data"), "QQ_CacheData"),
            new("QQ Code Cache", Path.Combine(roaming, @"QQ\Code Cache"), "QQ_CodeCache"),
            new("QQ GPUCache", Path.Combine(roaming, @"QQ\GPUCache"), "QQ_GPUCache"),
            new("QQ DawnGraphiteCache", Path.Combine(roaming, @"QQ\DawnGraphiteCache"), "QQ_DawnGraphiteCache"),
            new("QQ DawnWebGPUCache", Path.Combine(roaming, @"QQ\DawnWebGPUCache"), "QQ_DawnWebGPUCache"),
            new("QQ Local Storage", Path.Combine(roaming, @"QQ\Local Storage"), "QQ_LocalStorage", false, "可能包含客户端登录态或本地配置，确认可迁移后再勾选。"),
            new("QQ Network Cache", Path.Combine(roaming, @"QQ\Network"), "QQ_Network"),
            new("QQ Shared Dictionary Cache", Path.Combine(roaming, @"QQ\Shared Dictionary\cache"), "QQ_SharedDictionary_Cache"),
            new("QQ 分区 Cache", Path.Combine(roaming, @"QQ\Partitions\*\Cache"), "QQ_Partitions_Cache"),
            new("QQ 分区 Code Cache", Path.Combine(roaming, @"QQ\Partitions\*\Code Cache"), "QQ_Partitions_CodeCache"),
            new("QQ 分区 GPUCache", Path.Combine(roaming, @"QQ\Partitions\*\GPUCache"), "QQ_Partitions_GPUCache"),
            new("QQ 分区 DawnGraphiteCache", Path.Combine(roaming, @"QQ\Partitions\*\DawnGraphiteCache"), "QQ_Partitions_DawnGraphiteCache"),
            new("QQ 分区 DawnWebGPUCache", Path.Combine(roaming, @"QQ\Partitions\*\DawnWebGPUCache"), "QQ_Partitions_DawnWebGPUCache"),
            new("QQ 分区 Network Cache", Path.Combine(roaming, @"QQ\Partitions\*\Network"), "QQ_Partitions_Network"),
            new("QQ 分区 blob_storage", Path.Combine(roaming, @"QQ\Partitions\*\blob_storage"), "QQ_Partitions_BlobStorage"),
            new("QQ 分区 Shared Dictionary Cache", Path.Combine(roaming, @"QQ\Partitions\*\Shared Dictionary\cache"), "QQ_Partitions_SharedDictionary_Cache"),
            new("QQEX Cache", Path.Combine(roaming, @"QQEX\Cache"), "QQEX_Cache"),
            new("QQEX Cache_Data", Path.Combine(roaming, @"QQEX\Cache\Cache_Data"), "QQEX_CacheData"),
            ...
            new("QQEX DawnGraphiteCache", Path.Combine(roaming, @"QQEX\DawnGraphiteCache"), "QQEX_DawnGraphiteCache"),
            new("QQEX DawnWebGPUCache", Path.Combine(roaming, @"QQEX\DawnWebGPUCache"), "QQEX_DawnWebGPUCache"),
            new("QQEX IndexedDB", Path.Combine(roaming, @"QQEX\IndexedDB"), "QQEX_IndexedDB", false, "可能包含小程序数据或本地配置，确认可迁移后再勾选。"),
            new("QQEX miniapp", Path.Combine(roaming, @"QQEX\miniapp"), "QQEX_miniapp", false, "可能包含 QQ 小程序数据，不建议默认迁移。"),
            new("QQEX Shared Dictionary Cache", Path.Combine(roaming, @"QQEX\Shared Dictionary\cache"), "QQEX_SharedDictionary_Cache"),
            new("QQEX 分区 Cache", Path.Combine(roaming, @"QQEX\Partitions\*\Cache"), "QQEX_Partitions_Cache"),
            new("QQEX 分区 Code Cache", Path.Combine(roaming, @"QQEX\Partitions\*\Code Cache"), "QQEX_Partitions_CodeCache"),
            new("QQEX 分区 GPUCache", Path.Combine(roaming, @"QQEX\Partitions\*\GPUCache"), "QQEX_Partitions_GPUCache"),
            new("QQEX 分区 DawnGraphiteCache", Path.Combine(roaming, @"QQEX\Partitions\*\DawnGraphiteCache"), "QQEX_Partitions_DawnGraphiteCache"),
            new("QQEX 分区 DawnWebGPUCache", Path.Combine(roaming, @"QQEX\Partitions\*\DawnWebGPUCache"), "QQEX_Partitions_DawnWebGPUCache"),
            new("QQEX 分区 Network Cache", Path.Combine(roaming, @"QQEX\Partitions\*\Network"), "QQEX_Partitions_Network"),
            new("QQEX 分区 blob_storage", Path.Combine(roaming, @"QQEX\Partitions\*\blob_storage"), "QQEX_Partitions_BlobStorage"),
            new("QQEX 分区 Shared Dictionary Cache", Path.Combine(roaming, @"QQEX\Partitions\*\Shared Dictionary\cache"), "QQEX_Partitions_SharedDictionary_Cache"),
            new("QQEX 用户 Cache", Path.Combine(roaming, @"QQEX\users\*\Cache"), "QQEX_Users_Cache"),
            new("QQEX 用户 Code Cache", Path.Combine(roaming, @"QQEX\users\*\Code Cache"), "QQEX_Users_CodeCache"),
            new("QQEX 用户 GPUCache", Path.Combine(roaming, @"QQEX\users\*\GPUCache"), "QQEX_Users_GPUCache"),
            new("QQEX 用户 Network Cache", Path.Combine(roaming, @"QQEX\users\*\Network"), "QQEX_Users_Network"),
            new("QQEX 用户 blob_storage", Path.Combine(roaming, @"QQEX\users\*\blob_storage"), "QQEX_Users_BlobStorage"),
            new("QQNT Cache", Path.Combine(roaming, @"Tencent\QQNT\Cache"), "Tencent_QQNT_Cache"),
            new("QQNT STemp", Path.Combine(roaming, @"Tencent\QQNT\STemp"), "Tencent_QQNT_STemp"),
            new("QQ STemp", Path.Combine(roaming, @"Tencent\QQ\STemp"), "Tencent_QQ_STemp"),
            ...
            new("QQ 文件缓存 Image", Path.Combine(documents, @"Tencent Files\*\Image"), "TencentFiles_Image", false, "聊天图片目录可能包含要保留的个人文件，不会默认勾选。"),
            new("QQ 文件缓存 Video", Path.Combine(documents, @"Tencent Files\*\Video"), "TencentFiles_Video", false, "聊天视频目录可能包含要保留的个人文件，不会默认勾选。"),
            new("QQ 文件缓存 FileRecv", Path.Combine(documents, @"Tencent Files\*\FileRecv"), "TencentFiles_FileRecv", false, "文件接收目录通常是个人文件，不会默认勾选。"),
